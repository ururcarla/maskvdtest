...
def _bbox_area(bbox):
  return (bbox[2] - bbox[0])*(bbox[3] - bbox[1]) # w*h

cats = ['Pedestrian', 'Car', 'Cyclist', 'Van', 'Truck',  'Person_sitting',
        'Tram', 'Misc', 'DontCare']
# mapping to coco
# cat_ids = {'Pedestrian': 1, # person,
#            'Car': 3, # car
#            'Cyclist': 2, # cycle
#            'Van': 3, # car
#            'Truck': 8, # truck
#            'Person_sitting': 1, # person
#            'Tram': 7, # train
#            'Misc': 91,
#            'DontCare': 0}
...
for i, cat in enumerate(cats):
  cat_info.append({'name': cat, 'id': cat_ids[cats.index(cat) + 1] if cat != 'Person' else cat_ids[cats.index('Person_sitting') + 1]})
    ...
    for i in VIDEO_SETS[split]:
      ...
      for ann_ind, txt in enumerate(anns):
        ...
        if tmp[2] == 'Person':
          ...
          cat_id = cat_ids[cats.index(tmp[2]) + 1]
