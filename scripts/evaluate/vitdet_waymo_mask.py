...
def build_waymo_dataset(config):
    dataset_cfg = config.get("dataset") or {}
    if not dataset_cfg:
        raise ValueError("配置中缺少 dataset 节点，无法构建 Waymo 数据集。")
...
def val_pass(device, model, data, config, output_file):
        ...
        for frame, annotations in vid_item:
            with torch.inference_mode():
                if step % config["period"] == 0:
                    ...
                    mask_index, window_index, sparsity, n_tokens = get_region_mask_dynamic(
                        results,
                    ...
                    if config["sparsity"] < 1.0:
                        mask_index = torch.unique(torch.cat((mask_index, mask_index_static), dim=1)).reshape(
                            1, -1
                        )
    ...
    if config["model"]["backbone_config"]["backbone"] == "windowed":
        ...
        mask_index_test, _ = model.get_region_mask_static(region_sparsity=1 - total_sparsity / count)
...
def main():
    ...
    val_data = build_waymo_dataset(config)
