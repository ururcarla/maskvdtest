...
def topkmask(feature, sparsity=0.5):
    ...
    _, idx = torch.topk(feature_flat, left_tokens, dim=1, sorted=False)
    return idx
...
def val_pass(device, model, data, config, output_file):
    ...
    n_frames = min(n_items, total_batches)

    starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
    warmup_iters = min(10, total_batches)
    model.reset()
    for _, (frame, annotations) in tqdm(zip(range(warmup_iters), data_loader), total=warmup_iters, ncols=0):
    ...
    dynamic_mask_cfg = config.get("dynamic_mask", {})
    dynamic_mask_enabled = dynamic_mask_cfg.get("enabled", True)

    heatmap_mask_cfg = config.get("heatmap_mask", {})
    heatmap_enabled = heatmap_mask_cfg.get("enabled", True)
    heatmap_window = int(config.get("dynamic_heatmap_window", 30))
    heatmap_min_frames = int(
        config.get(
            "dynamic_heatmap_min_frames",
            max(1, min(heatmap_window, 30)),
        )
    )
    heatmap_min_activity = float(config.get("dynamic_heatmap_min_activity", 0.0))
    heatmap_score_threshold = float(config.get("dynamic_heatmap_score_threshold", 0.75))
    heatmap_keep_ratio = max(0.0, min(1.0, 1 - target_sparsity)) if heatmap_enabled else 0.0

    safety_cfg = config.get("safety", {})
    safety_enabled = safety_cfg.get("enabled", False)
    ...
    for _, (frame, annotations) in tqdm(zip(range(n_items), data_loader), total=n_items, ncols=0):
        annotation_list = list(annotations) if isinstance(annotations, (list, tuple)) else [annotations]
        frame_meta = annotation_list[0] if annotation_list else {}
        frame_id = frame_meta.get("frame_id", 0)
        ...
        with torch.inference_mode():
            ...
            if (
                ...
                if heatmap_state is not None and heatmap_mask_cache is not None:
            ...
            if run_model:
                ...
                if heatmap_state is not None:
                    ...
                    new_heatmap_mask = heatmap_state.build_mask_index(
                        keep_ratio=heatmap_keep_ratio,
                        min_ready_frames=heatmap_min_frames,
                        min_activity=heatmap_min_activity,
                    )
        ...
        labels.extend([dict_to_device(annotation, device) for annotation in annotation_list])
    ...
    if config["model"]["backbone_config"]["backbone"] == "windowed":
        ...
        mask_index_test, _ = model.get_region_mask_static(region_sparsity=1 - avg_model_sparsity)
