...
class SlidingWindowHeatmap:
    """统计最近窗口内的检测结果并生成稀疏mask索引。"""

    def __init__(self, image_shape, region_size, window_size, score_threshold=0.5):
        self.image_shape = tuple(image_shape)
        self.region_size = max(1, int(region_size))
        self.window_size = max(1, int(window_size))
        self.score_threshold = 0.0 if score_threshold is None else float(score_threshold)
        self.grid_height = max(1, self.image_shape[0] // self.region_size)
        self.grid_width = max(1, self.image_shape[1] // self.region_size)
...
def val_pass(device, model, data, config, output_file):
    ...
    model.clear_counts()
    # -------------------------------------------------------------------------
    region_size = int(config.get("region_size", 16))
    heatmap_window = int(config.get("dynamic_heatmap_window", 30))
    heatmap_min_frames = int(
        config.get("dynamic_heatmap_min_frames", max(1, min(heatmap_window, 30)))
    )
    heatmap_min_activity = float(config.get("dynamic_heatmap_min_activity", 0.0))
    heatmap_score_threshold = float(
        config.get("dynamic_heatmap_score_threshold", config.get("conf", 0.5))
    )
    heatmap_keep_ratio = max(0.0, min(1.0, 1 - float(config.get("sparsity", 0.0))))
        ...
        for frame, annotations in vid_item:
            with torch.inference_mode():
                ...
                if is_key_frame:
                    ...
                    mask_index, _, _, n_tokens = get_region_mask_dynamic(
                        results,
                    ...
                    if (
                        heatmap_mask_cache is not None
                        and heatmap_keep_ratio > 0.0
                    ):
                        mask_index = torch.unique(
                            torch.cat((mask_index, heatmap_mask_cache), dim=1)
                        ).reshape(1, -1)
